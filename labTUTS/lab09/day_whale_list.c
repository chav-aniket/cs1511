...
//Made by Aniket Chavan (z5265106) (chavaniket@outlook.com) on 21-04-19
#include <string.h>
#include <stdlib.h>
...
int compare_dates(struct pod *current, struct date *day);
...
//
int compare_dates(struct pod *current, struct date *day) {
    if (current->when->day == day->day && current->when->month == day->month
        && current->when->year == day->year) {
        return 1;
    }
    return 0;
}
...
void day_whales(struct pod *first_pod, struct date *day) {
    ...
    while (current != NULL) {
        if (compare_dates(current, day)) {
            printf("%d %s\n", current->how_many, current->species);
        }
