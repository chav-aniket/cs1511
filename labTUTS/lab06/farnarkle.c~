...
//
// This program by Aniket Chavan (z5265106) on 29-03-19
...
int count_arkles(int tiles1[N_TILES], int tiles2[N_TILES]) {
    ...
    for (int i = 0;i<N_TILES;i++) {
        for (int j = 0;j<N_TILES;j++) {
            if (tiles1[i] == tiles2[j] && tiles1[j]!=tiles2[j] && 
                tiles1[i]!=tiles2[i]) {
                sum++;
                tiles2[j] *= -1;
                break;
...
void play_farnarkle(int hidden_tiles[N_TILES]) {
    ...
    while (!win) {
        ...
        printf("%d farnarkles, %d arkles\n",count_farnarkles(array,hidden_tiles),count_arkles(array,hidden_tiles));
        turn++;
        if(count_farnarkles(array,hidden_tiles) == 4) {
            win = 1;
        }
    }
    printf("You win\n");
